...
const REQUIREMENT_LINE = /^[A-Za-z0-9_.-]+([<>=!~]=?[A-Za-z0-9_.!*+,<>= -]+)?(\s*;[A-Za-z0-9_ ."'()<>=!-]+)?$/;
...
export function validatePythonRequirementsText(text: string): string {
  ...
  for (const line of normalizedLines) {
    if (UNSAFE_REQUIREMENT.test(line) || !REQUIREMENT_LINE.test(line)) {
      throw new Error("requirements.txt only supports package names and version specifiers");
    }
  }
