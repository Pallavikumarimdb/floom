...
function getAppContract(): McpToolResult {
  ...
  return okResult({
    ...
    unsupported: [
      ...
      {
        case: "long-running sync jobs beyond 290 seconds",
        reason: "sync cap is 290s (?wait=true); for jobs that may exceed this, use async mode (POST without ?wait=true, then poll GET /api/executions/<id> until terminal status).",
      },
      {
    ...
    auth_and_access: {
      ...
      connections: "Floom supports Composio-backed OAuth integrations (Gmail, Slack, GitHub, Notion, and 40+ others). Use list_my_connections to see which services the owner has already authorized. To authorize a new service, direct the user to https://floom.dev/integrations and have them click Connect next to the desired provider. Once authorized, the connection ID is available as COMPOSIO_CONNECTION_ID in the app environment; read it with os.environ.get('COMPOSIO_CONNECTION_ID') and pass it to Composio SDK calls. Connections are per-user and persist across app runs.",
    },
    setup_commands: [
    ...
    async_calling: asyncEnabled
      ...
      : "Async runtime is not enabled on this Floom deployment. Call POST /api/apps/:slug/run?wait=true (or omit async) for synchronous runs within the 290 s execution cap. Async poll pattern (POST without ?wait=true then GET /api/executions/<id>) is available on deployments with async runtime enabled.",
    run_tool: {
      name: "run_app",
