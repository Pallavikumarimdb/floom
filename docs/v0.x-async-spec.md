...
- `docs/rls-audit.md` confirms current execution reads are enforced in the API layer because the service-role admin client bypasses RLS
...
- private app without auth: `403`
- private app with wrong owner token: `403`
...
Post-merge rollout required by the task:
1. one day on canonical with internal apps only
2. then flip for the public-secret demo
3. then broaden to all public apps after a clean canary

During the internal-only day, watch:
- queued depth
- stuck leases
